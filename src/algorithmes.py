...
from parser_dzn import parse_all_instances
...
class Algorithmes:
    ...
    def lpt(self):
        ...
        """
        # Filtrer les activités avec durée > 0 (ignorer les activités fictives)
        # Attention: les indices des activités commencent à 1,
        # mais les indices de la liste durations commencent à 0
        valid_activities = [a for a in self.activities if self.durations[a-1] > 0]
    ...
    def mst(self):
        ...
        try:
            ...
            if nx.is_connected(G):
                ...
                # Trouver une racine appropriée
                # (préférer l'activité avec le plus petit indice)
                root = min(G.nodes())
                ...
                # Réordonner en respectant les contraintes de précédence
                ordered_activities = self._respect_precedence(
                    ordered_activities
                    )
