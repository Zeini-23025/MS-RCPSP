...
def afficher_statistiques_methodes(results_dir):
    """
    Affiche les statistiques sur le pourcentage
    de solutions pour chaque méthode
    ...
    try:
        ...
        print(
            f"{'Méthode':<15} | {'Nombre d\'instances':<20} | "
            f"{'Pourcentage':<15}"
        )
        print("-"*60)
        ...
        print(
            f"La méthode la plus efficace est {best_algo} avec "
            f"{algo_counts[best_algo]} instances "
            f"({(algo_counts[best_algo]/total_instances)*100:.2f}%)"
        )
        print("="*60 + "\n")
        ...
        try:
            ...
            plt.title(
                'Nombre d\'instances où chaque algorithme '
                'est le meilleur'
            )
            ...
            print(f"Graphique enregistré dans {os.path.join(
                results_dir,
                'statistiques_methodes.png'
                )}")
            plt.close()
